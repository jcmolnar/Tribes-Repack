...
// No-Crash-Mod (For lack of a better name and creativity) 
// By plasmatic, not you.
// Refined 3/15/2006 3:08PM
...
function remoteSetInfoLine(%mgr, %lineNum, %text)
{
   if(%mgr != 2048)
	...
	%lineNum = floor(%lineNum);
	if(string::getSubStr(%text, 0, 250) != %text)
		return false;
...
function onClientMessage(%client, %msg)
{
   ...
   // filter messages here
	if(string::getSubStr(%msg, 0, 250) == %msg)
	{
		return true;
	}
}
...
{
//	if(string::findSubStr(%msg, ".bmp>") != "-1" || string::findSubStr(%msg, "<R") != "-1" || string::findSubStr(%msg, "<L") != "-1" || string::findSubStr(%msg, "<S") != "-1")
//		return;	
//	if(!evalspam())return;
	if(string::getSubStr(%msg, 0, 500) != %msg)
		%msg = string::getSubStr(%msg, 0, 500);
   if(%manager == 2048)
...
function show()
{
   ...
   setWindowTitle(MainWindow, "Tribes");
}
show();
...
function showFPS()
{
   function Game::EndFrame()
   {
	   setWindowTitle(MainWindow, "Tribes - "@$ConsoleWorld::FrameRate);
...
{
	if(string::getSubStr(%wav, 0, 50) != %wav)
		return false;
	%anim = floor(%anim);
...
function remoteSVInfo(%server, %version, %hostname, %mod, %info, %favKey)
{
   if(%server == 2048)
   {
   	if(string::getSubStr(%version, 0, 250) != %version)
		return false;
	if(string::getSubStr(%hostName, 0, 250) != %hostName)
		return false;
	if(string::getSubStr(%mod, 0, 250) != %mod)
		return false;
	//if(string::getSubStr(%info, 0, 250) != %info)
	//	return false;
	if(string::getSubStr(%favKey, 0, 250) != %favKey)
		return false;
...
function remoteMODInfo(%server, %modString)
{
	...
	if($ServerModInfo != "")
	{		%msg = %modString;
		if($joiningWorlds){
			...
			%msg = "<jc><f1>NOTICE: This server has been modified\n<f0>" @ %msg;
		Control::setValue(ModTextString, %msg);
	}
}
...
function remoteMInfo(%server, %missionName)
{
   if(%server == 2048)
   {
   	if(string::getSubStr(%missionName, 0, 220) != %missionName)
	%missionName = string::getSubStr(%missionName, 0, 220);
...
function remoteMissionChangeNotify(%serverManagerId, %nextMission)
{
   ...
   {
  	if(string::getSubStr(%nextMission, 0, 220) != %nextMission)
		%nextMission = string::getSubStr(%nextMission, 0, 220);
      cls();
      pecho("Server mission complete - changing to mission: ", %nextMission);
