//NEW//
$team_search_str[1]="Reb";
$team_search_str[2]="Imp";
$team_search_str[3]="";
//$team_search_str[3]="Generic1";
//$team_search_str[4]="Generic2";
//$team_search_str[5]="Generic3";
//$team_search_str[6]="Generic4";

function team_item(%clientId,%item)
{
	%found=0;
	%team_item=0;
	%player_team=Client::getTeam(%clientId);
	for(%i=1;$team_search_str[%i]!="";%i++)
	{
		if(String::findSubStr(%item.heading,$team_search_str[%i])!=-1)
		{
			%found=1;
			if(%i-1==%player_team)
			{
...
//----------------------------------------------------------------------------
// Rebels = team 0 (blood eagle)
// Imperials = team 1 (diamond sword)
//----------------------------------------------------------------------------

// List of all items available to buy from inventory station
$InvList[ForceThrow] = 1;      // both teams
$InvList[ForceLightning] = 1;  // both teams
$InvList[ForceSpeed] = 1;  // both teams
$InvList[BSaber] = 1; // rebel
$InvList[RSaber] = 1; // imp
$InvList[GSaber] = 1; // rebel
$InvList[MSaber] = 1; // imp
$InvList[Blaster] = 1; // imp
$InvList[TBlaster] = 1; // imp
$InvList[BBlaster] = 1; // imp
$InvList[HBlaster] = 1; // rebel
$InvList[NBlaster] = 1; // both teams
$InvList[ScoutGun] = 1; // imp
$InvList[BlasterRifle] = 1; // imp
$InvList[GuardGun] = 1; // both
$InvList[DesertRifle] = 1; // imp
$InvList[Repeater] = 1; // both
$InvList[TorpLauncher] = 1; // both
$InvList[FlakCannon] = 1; // both
$InvList[DeployableInvPack] = 1; // both
$InvList[DeployableAmmoPack] = 1; // both




$InvList[TimerMineAmmo] = 1; // imp

$InvList[FlakAmmo] = 1;        // both teams
$InvList[PTorpedoAmmo] = 1;        // both teams
$InvList[MineAmmo] = 1;        // both teams
$InvList[Grenade] = 1;         // both teams
$InvList[ThermalDet] = 1;      // both teams

$InvList[BlasterAmmo] = 1;     // both teams
$InvList[BlasterRifleAmmo] = 1;    // imp
$InvList[RepeaterAmmo] = 1; // both teams
$InvList[GuardGunAmmo] = 1; // both teams

  
$InvList[EnergyPack] = 1; // both teams
$InvList[RepairPack] = 1; // both teams
$InvList[ShieldPack] = 1;  // both teams
$InvList[CameraPack] = 0;  // both teams
$InvList[TurretPack] = 1;  // both teams
$InvList[RepairKit] = 1;  // both teams

//--------------------------------------------------------------------
// List of all Vehicles available to buy from Vehicle Station
...
$RemoteInvList[BSaber] = 1; // rebel
$RemoteInvList[RSaber] = 1; // imp
$RemoteInvList[GSaber] = 1; // rebel
$RemoteInvList[MSaber] = 1; // imp
$RemoteInvList[Blaster] = 1; // imp
$RemoteInvList[TBlaster] = 1; // imp
$RemoteInvList[BBlaster] = 1; // imp
$RemoteInvList[HBlaster] = 1; // rebel
$RemoteInvList[NBlaster] = 1; // both teams
$RemoteInvList[ScoutGun] = 1; // imp
$RemoteInvList[BlasterRifle] = 1; // imp
$RemoteInvList[GuardGun] = 1; // both
$RemoteInvList[DesertRifle] = 1; // imp
$RemoteInvList[Repeater] = 1; // both
$RemoteInvList[TorpLauncher] = 1; // both
$RemoteInvList[FlakCannon] = 1; // both

$RemoteInvList[TimerMineAmmo] = 1; // imp
$RemoteInvList[FlakAmmo] = 1;        // both teams
$RemoteInvList[PTorpedoAmmo] = 1;        // both teams
$RemoteInvList[MineAmmo] = 1;        // both teams
$RemoteInvList[Grenade] = 1;         // both teams
$RemoteInvList[ThermalDet] = 1;      // both teams
$RemoteInvList[BlasterAmmo] = 1;     // both teams
$RemoteInvList[BlasterRifleAmmo] = 1;    // imp
$RemoteInvList[RepeaterAmmo] = 1; // both teams
$RemoteInvList[GuardGunAmmo] = 1; // both teams

$RemoteInvList[EnergyPack] = 1; // both teams
$RemoteInvList[RepairPack] = 1; // both teams
$RemoteInvList[ShieldPack] = 1;  // both teams
$RemoteInvList[CameraPack] = 0;  // both teams
$RemoteInvList[TurretPack] = 1;  // both teams
$RemoteInvList[RepairKit] = 1;  // both teams
		...
		if($TeamEnergy[%team] != "Infinite") {
			if(%this.clTeamEnergy != %client.TeamEnergy) {
				if(%client.teamEnergy < 0)
					Client::sendMessage(%client,0,"Your total mission purchases have come to " @
(%client.teamEnergy * -1) @ ".");
				else
					Client::sendMessage(%client,0,"You have increased the Team Energy by " @ %client.teamEnergy
@ ".");
			}
			if((%client.teamEnergy -%client.EnergyWarning < $TeammateSpending) && ($TeammateSpending !=
0) && !$TeamEnergyCheat) {
				TeamMessages(0, %team, "Teammate " @ Client::getName(%client) @ " has spent " @
(%client.teamEnergy *-1) @ " of the TeamEnergy"); 
 			...
 			if(GameBase::getTeam(%object) == GameBase::getTeam(%this) || GameBase::getTeam(%this) == -1)
{
   	   ...
   	   {
				%curTime = getSimTime();
				if(%curTime - %object.stationDeniedStamp > 3.5 && GameBase::getDamageState(%this) ==
"Enabled") {
...
function Station::itemsToResupply(%player)
{
	...
	%cnt = %cnt + AmmoStation::resupply(%player,"",Grenade,4);
	%cnt = %cnt + AmmoStation::resupply(%player,"",RepairKit,1);
	%cnt = %cnt + AmmoStation::resupply(%player,"",MineAmmo,3);
	%cnt = %cnt + AmmoStation::resupply(%player,"",TimerMineAmmo,3);
	%cnt = %cnt + AmmoStation::resupply(%player,Blaster,BlasterAmmo,200);
	%cnt = %cnt + AmmoStation::resupply(%player,TBlaster,BlasterAmmo,200);
	%cnt = %cnt + AmmoStation::resupply(%player,BBlaster,BlasterAmmo,200);
	%cnt = %cnt + AmmoStation::resupply(%player,HBlaster,BlasterAmmo,200);
	%cnt = %cnt + AmmoStation::resupply(%player,NBlaster,BlasterAmmo,200);
	%cnt = %cnt + AmmoStation::resupply(%player,FlakCannon,FlakAmmo,50);
	%cnt = %cnt + AmmoStation::resupply(%player,TorpLauncher,PTorpedoAmmo,6);
	%cnt = %cnt + AmmoStation::resupply(%player,Repeater,RepeaterAmmo,200);
	%cnt = %cnt + AmmoStation::resupply(%player,DesertRifle,DesertRifleAmmo,20);
	%cnt = %cnt + AmmoStation::resupply(%player,BlasterRifle,BlasterRifleAmmo,50);
	%cnt = %cnt + AmmoStation::resupply(%player,GuardGun,GuardGunAmmo,50);
	return %cnt;
}
...
function AmmoStation::onResupply(%this)
{
	if (GameBase::isActive(%this)) {
		...
		if (%player != -1 && %this.lastPlayer == %player) {
			...
			Client::setInventoryText(%client, "<f1><jc>TEAM ENERGY: " @
$TeamEnergy[Client::getTeam(%client)]);
...
function InventoryStation::onResupply(%this,%InvShopList)
{
	...
	if (%this.target != "") {	   
		...
		if(GameBase::getDataName(%player.Station) == DeployableInvStation) {
			Client::setInventoryText(%this.target, "<f1><jc>TEAM ENERGY: " @
$TeamEnergy[Client::getTeam(%this.target)]);
   ...
   Client::clearItemBuying(%client);
	...
	for (%i = 0; %i < %max; %i++) {
      ...
      if(!%item.showInventory)
		...
		else if(%item.className == Armor && %item != $ArmorName[%armor] &&
Client::isItemShoppingOn(%client,%i)) 
			Client::setItemBuying(%client, %item);
		else if(%item.className == Vehicle && $TeamItemCount[client::getTeam(%client) @ %item] <
$TeamItemMax[%item] && Client::isItemShoppingOn(%client,%i))
			Client::setItemBuying(%client, %item);
	}
}
		...
		if(!%objInWay) {
            ...
            %vehicle.clLastMount = %client;
				...
				if(%object.fading) { 
					...
					schedule("GameBase::setRotation(" @ %vehicle @ ",\"" @ GameBase::getRotation(%obj) @
"\");",2.5,%vehicle);
					schedule(%vehicle @ ".fading = \"\"; VehiclePad::checkSeq(" @ %obj @ "," @ %player.Station
@ ");",5,%vehicle);
