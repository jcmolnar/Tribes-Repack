...
// NATIVE: scan the WHOLE search path (every mounted vol/zip + loose subfolder) for skin
// textures matching %pattern (e.g. "*.larmor.png") and append each UNIQUE skin to
// $SkinArray. %delim is ".larmor" (male) or ".lfemale" (female) -- the dedup key is the
// skin base + gender, so a skin shipped as both .bmp and .png (repack conversion) only
// lists once, while male/female of the same base stay distinct. This makes the skin
// dropdown find skins in ANY folder and ANY image format, not just root .bmp.
...
function PlayerSetupGui::onOpen()
{
   ...
   for(%i = 0; $PCFG::Name[%i] != ""; %i++)
	...
	//image format the repack ships (legacy .bmp + converted .png/.gif). Male = *.larmor.*,
	//female = *.lfemale.*; the combo below gender-filters on the ".larmor"/".lfemale"
	//substring. Dedup by base+gender (Player::addSkinFiles). Clear the array first so a
	//prior open with a longer list can't leave stale trailing entries.
	%i = 0;
	while (%i < 256)
	{
		$SkinArray[%i] = "";
...
function AddPlayerConfig(%newPlayer)
{
	...
	if (%newPlayer != "")
	{
		...
		else
		{
			...
			$PCFG::SkinBase[$PCFG::LastPlayer]	= "rpgbase";
