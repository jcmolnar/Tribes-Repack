...
}function clearmenustuff(%clientId){	for(%i = 0; %clientId.tabmenuoption[%i] != ""; %i++)		%clientId.tabmenuoption[%i] = "";}
...
function remoteMenuSelect(%clientId, %code)
{
	...
	if(String::findSubStr(%code, "\"") != -1 ||
		String::findSubStr(%code, "\\") != -1)  // no quotes or escapes
			return;
	%pass = False;	for(%i = 0; %clientId.tabmenuoption[%i] != ""; %i++){		if(%code == %clientId.tabmenuoption[%i])			%pass = True;	}	if(!%pass){		pecho("Menu cheating "@%clientId@" "@%code);		return;	}
