...
//	FGCombo::addEntry(OptionsVideo::OpenGLCombo, "G200/G400", 3);
	...
	else if ($pref::VideoOpenGLMode == "Rage 128" || $pref::VideoOpenGLMode == "Generic")
	{
	...
	%v = Control::getValue(IDCTG_OPTS_VID_FULLSCRN);
	%temp = getWord(%v, 0);
	if (%temp == "OpenGL")
	{
		$pref::VideoDriverName = %v;
...
function OptionsVideo::validate()
{
   ...
   if ($pref::VideoFullScreenDriver == "")
   {
      ...
      else if (isGfxDriver(MainWindow, "OpenGL")) $pref::VideoFullScreenDriver = "OpenGL";
...
function OptionsMovement::init()
{
   ...
   Control::setValue(IDCTG_OPTS_CFG_MOUSE_SENSITIVE, 0.3);//%percentage);
   Control::setValue(IDCTG_OPTS_CFG_MOUSE_X_FLIP, FALSE);//Client::getMouseXAxisFlip("playMap.sae"));
   Control::setValue(IDCTG_OPTS_CFG_MOUSE_Y_FLIP, FALSE);//Client::getMouseYAxisFlip("playMap.sae"));
...
function OptionsNetwork::validate()
{
   ...
   // KRONOS: modem-era caps raised to the engine's fiber limits (rate 100/s, size 1000B =
   // the 10-bit wire max). The negotiation takes the server's conservative side, so this is
   // wire-compatible with unmodified servers. Was: rate<=30, size<=500 — which silently
   // re-clamped ClientPrefs every boot.
   if ($pref::packetRate < 1) $pref::packetRate = 60;
   if ($pref::packetRate > 100) $pref::packetRate = 100;
   Control::setValue(NetworkPacketRate, $pref::packetRate);

   if ($pref::packetSize < 100) $pref::packetSize = 1000;
