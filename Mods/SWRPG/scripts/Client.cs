...
function use(%d) //%d was %desc, changed to %d for shortness.
{
	if (%d == "Repair Kit"
	 || %d == "Beacon"
	 || %d == "Backpack"
	 ...
	 || %d == "Mortar"
	 || %d == "Targeting Laser")
		remoteEval(2048,useItem,%d);

//	%type = getItemType(%d);
	else if (getItemType(%d) != -1)
	{
	...
	//else if(%d == "Disc Launcher" || %d == "Grenade Launcher" || %d == "Laser Rifle" || %d == "ELF gun" || %d == "Mortar" || %d == "Targeting Laser")
	//	remoteEval(2048,chatbinds,%d);
	else
		echo("Unknown item \"" @ %d @ "\"");
}
...
{
   if(%pctDone < 0.1)
      %text = "Boarding transport";
   else if(%pctDone < 0.2)
      %text = "Transport taking off";
   else if(%pctDone < 0.3)
      %text = "Transport flying";
   else if(%pctDone < 0.4)
      %text = "Over salted space peanuts served";
   else if(%pctDone < 0.5)
      %text = "Undersized drinks served";
   else if(%pctDone < 0.6)
      %text = "*Boredom*";
   else if(%pctDone < 0.7)
      %text = "Transport landing";
   else if(%pctDone < 0.8)
      %text = "Exiting transport...";
   else
...
function dataFinished()
{
   ...
   Control::setValue(ProgressText, "<jc><f0>Enjoy your stay on " @ $MissionName @ "!");
