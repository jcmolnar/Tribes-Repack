...
{
	echo("STATION: Collision (" @ %this @ "," @ %object @ ")");
...
function Computer::onCollision(%this, %object)
{
	%obj = getObjectType(%object);
	if (%obj == "Player") {
  	 	%clientId = Player::getClient(%object);
...
//----------------------------------------------------------------------------
StaticShapeData Computer1
{
   description = "Computer Station";
	shapeFile = "cmdpnl";
	className = "Station";
	visibleToSensor = true;
        maxDamage = 999999999999.0;
...
function Computer1::onEndSequence(%this,%thread)
{
	if (Station::onEndSequence(%this,%thread)) 
		Computer::Initialize(player::getclient(%this.target), %this);
	(Client::getOwnedObject(%this.target)).Station = "";
	%this.target = "";
}
function Computer2::onEndSequence(%this,%thread)
{
	if (Station::onEndSequence(%this,%thread)) 
		Computer::Initialize(player::getclient(%this.target), %this);
	(Client::getOwnedObject(%this.target)).Station = "";
...
//----------------------------------------------------------------------------
StaticShapeData Computer2
{
   description = "Computer Terminal";
	shapeFile = "vehi_pur_pnl";
	className = "Station";
	visibleToSensor = true;
//   explosionId = DebrisExp;
