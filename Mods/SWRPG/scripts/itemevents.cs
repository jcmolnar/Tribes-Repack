...
function Item::onCollision(%this,%object)
{
	...
	if(getObjectType(%object) == "Player" && !IsDead(%clientId))
	{
		...
		else
		{
			...
			if(%item != "Lootbag" && Item::giveItem(%object, %item, %this.delta, True))
			{
				Item::playPickupSound(%this);
				RefreshAll(%clientId);
				Item::respawn(%this);
...
{
	if(%item == HikenLightsaber)
		Player::MountItem(%player, HikenHilt, 3);
}
...
function Item::onUse(%player,%item)
{
	...
	if(!IsDead(%clientId))
	{
		...
		if(%item.className == Accessory)
		{
			...
			if(SkillCanUse(%clientId, %item))
			{
				if(%cnt < $maxAccessory[GetAccessoryVar(%item, $AccessoryType)])
				{
					...
					schedule(%item @ "::onEquip(" @ %player @ ", " @ %item @ ");", 0.01);
				}
				else
		...
		else if(%item.className == Equipped)
		{
			...
			schedule(%item @ "::onUnequip(" @ %player @ ", " @ %item @ ");", 0.01);
