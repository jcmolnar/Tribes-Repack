...
function Player::onCollision(%this,%object)
{
	...
	//if(($Tackling || IsOnIce(%this)) && (!Player::isDead(%this) && GetObjectType(%object) == "Player" && getword(item::getVelocity(%object), 2) != 0)) //IsOnIce. Replace it with a getZone or whatever to find if the player's in the icerink for winter events.
	if($Tackling && !Player::isDead(%this) && GetObjectType(%object) == "Player" && getword(item::getVelocity(%object), 2) != 0)
	{
		Player::SetAnimation(%this, 33);
		Player::SetAnimation(%object, 25);
		%tacklee = Player::GetClient(%this);
		%tackler = Player::GetClient(%object);
		client::sendMessage(%tacklee, 0, Client::GetName(%tackler) @ " knocked you over!");
		client::sendMessage(%tackler, 0, "You knocked " @ Client::GetName(%tacklee) @ " over!");
	}
}
...
function Player::jump(%this,%mom)
{
	...
	messageAll(0, "Is this even called?" @ %mom);
