	...
	if(%group != -1)
	{
		...
		for(%i = 0; %i <= %count-1; %i++)
		{
			...
			else
			{
				...
				$Zone::Type[%zcnt] = %type; //echo(%desc @ ", " @ %type);
				$Zone::Desc[%zcnt] = %desc; echo(%desc @ ", " @ %tmpgroup @ ".");
...
function UpdateZone(%object)
{
	...
	else
	{
		...
		//Check if it's sandstorming! If it is, hurt 'em.
		if($IsSandStorming == True) //check if in #camp tent also
		{
			if($StormProgress > 60)
			{
				%dmg = Cap($StormProgress / 10, 1, 1000) * "0.01";
				GameBase::virtual(%clientId, "onDamage", 0, %dmg, "0 0 0", "0 0 0", "0 0 0", "torso", "front_right", %clientId);
				%snd = radnomItems(4, Cough1, Cough2, Cough3, Cough4);
				playSound(%snd, GameBase::getPosition(%clientId));
			}
		}
	}
