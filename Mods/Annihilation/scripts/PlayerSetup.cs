...
//------------------------------------------------------------------------------
// NATIVE-PORT (player-setup preview audit, 2026-07-29): the skin inventory.
//
// $SkinArray holds FILENAMES (the readers downstream do findSubStr(".larmor") and take the part
// before it), so a .png or .gif entry works exactly like the .bmp ones always did -- only the
// discovery pattern was stale.
//
// Deduplicated by LOGICAL skin, so a skin shipping as both base.larmor.png and base.larmor.bmp
// appears once. Format priority is PNG, then GIF, then BMP: PNG is the stock format now, and
// scanning in that order means the first one seen wins.
//
// The seen-set is keyed with a generation counter rather than being cleared, because console
// arrays have no clear operation and a stale key would silently suppress a skin on the next open.
//------------------------------------------------------------------------------
function PlayerSetup::addSkins(%pattern, %suffix)
{
	%skin = File::findFirst(%pattern);
	while (%skin != "")
	{
		%strIndex = String::findSubStr(%skin, %suffix);
		if (%strIndex >= 0)
		{
			%base = String::getSubStr(%skin, 0, %strIndex);
			%key = $SkinScanGen @ "/" @ %suffix @ "/" @ %base;
			if ($SkinSeen[%key] == "")
			{
				$SkinSeen[%key] = "1";
...
function PlayerSetup::buildSkinInventory()
{
	// Clear what a previous open left behind. Without this a scan that finds FEWER skins than the
	// last one leaves the old tail in place, and every reader here terminates on the first empty
	// entry -- so the stale entries would still be live.
	%i = 0;
	while ($SkinArray[%i] != "")
	{
		$SkinArray[%i] = "";
	...
	$SkinScanGen = $SkinScanGen + 1;
	$SkinCount = 0;

	PlayerSetup::addSkins("*.larmor.png", ".larmor");
	PlayerSetup::addSkins("*.larmor.gif", ".larmor");
	PlayerSetup::addSkins("*.larmor.bmp", ".larmor");

	PlayerSetup::addSkins("*.lfemale.png", ".lfemale");
	PlayerSetup::addSkins("*.lfemale.gif", ".lfemale");
	PlayerSetup::addSkins("*.lfemale.bmp", ".lfemale");

	// Terminator: every reader below walks until the first empty entry.
	$SkinArray[$SkinCount] = "";

	if ($SkinCount == 0)
	{
		// Never leave an empty frame with no explanation -- that is the symptom this whole fix
		// exists to remove.
		echo("[PlayerPreview] ERROR stage=inventory reason=\"no .larmor/.lfemale skins found on the search path (png, gif or bmp)\"");
	}
	else
	{
		if ($pref::playerPreviewDiag != "")
		{
			echo("[PlayerPreview] inventory unique=" @ $SkinCount);
		}
	}
}
...
function PlayerSetupGui::onOpen()
{
   ...
   for(%i = 0; $PCFG::Name[%i] != ""; %i++)
	...
	// NATIVE-PORT (player-setup preview audit, 2026-07-29): scan PNG and GIF as well as BMP.
	//
	// This searched ONLY "*.larmor.bmp" / "*.lfemale.bmp". The base tree's stock skins are PNG
	// (45 of them, inside base/Entities.zip) and there is not a single .larmor.bmp anywhere under
	// base\ -- every BMP in the install belongs to a mod (RPG, SWRPG, RMRPG, zsdev, War40k). So on
	// a no-mod client $SkinArray came back EMPTY, nothing ever called setSkinBase, SkinView::instance
	// stayed null, and Player Setup drew its frame around nothing.
	//
	// That is also why it went unnoticed for so long: on RPG/Kronos the mod's own BMP skins are on
	// the search path, so the screen works there and only base is broken.
	PlayerSetup::buildSkinInventory();
