...
// DELTAFORCE
$missionTypes[5, type]        = "Hostage";
$missionTypes[5, description] = "Rescue the Hostages";
$missionTypes[5, minTeams]    = 3;
$missionTypes[5, maxTeams]    = 8;
$missionTypes[5, varName, 0]  = scoreLimit;
$missionTypes[5, varDesc, 0]  = "Score limit:";
$missionTypes[5, varVal, 0]   = 300;
$missionTypes[5, varName, 1]  = cdTrack;
$missionTypes[5, varDesc, 1]  = "CD Track";
$missionTypes[5, varVal, 1]   = 2;
$missionTypes[5, varName, 2]  = cdMode;
$missionTypes[5, varDesc, 2]  = "CD Play Mode";
$missionTypes[5, varVal, 2]   = 1;

$missionTypes[6, type]        = "Assassination";
$missionTypes[6, description] = "Assassination";
$missionTypes[6, minTeams]    = 2;
$missionTypes[6, maxTeams]    = 2;
$missionTypes[6, varName, 0]  = scoreLimit;
$missionTypes[6, varDesc, 0]  = "Number of kills/escapes needed to win";
$missionTypes[6, varVal, 0]   = 25;
$missionTypes[6, varName, 1]  = escapePoints;
$missionTypes[6, varDesc, 1]  = "Number of escape locations";
$missionTypes[6, varVal, 1]   = 2;
$missionTypes[6, varName, 2]  = cdTrack;
$missionTypes[6, varDesc, 2]  = "CD Track";
$missionTypes[6, varVal, 2]   = 2;
$missionTypes[6, varName, 3]  = cdMode;
$missionTypes[6, varDesc, 3]  = "CD Play Mode";
$missionTypes[6, varVal, 3]   = 1;
// END DELTAFORCE
...
function Mission::Assassination::create(%numTeams) {	
	for (%i = 0; %i < $Assassination::escapePoints; %i++) {
		%escape = newObject("Escape Route" @ %i, SimGroup);
		addToSet("MissionGroup", %escape);
      	%point = newObject("Escape Point" @ %i, StaticShape, TowerSwitch);
      	%point.scoreValue = 1;
      	GameBase::setMapName(%point, "Escape Point " @ (%i + 1));
      	addToSet(%escape, %point);
	}

	// add lines to the mission file...
	addExportText("$teamScoreLimit = "@$Assassination::scoreLimit@";");
	addExportText("exec(objectives);");
	addExportText("$Game::missionType = \"Assassination\";");
	addExportText("$Hunted = 0;");
	addExportText("$HuntedQueue = 0;");
}
...
// DELTAFORCE
function Mission::Hostage::create(%numTeams)
{
   %aiSet = newObject(AI, SimGroup);
   addToSet("MissionGroup\\Teams\\team" @ %numTeams-1, %aiSet);

   // Add five hostages initially to the mission
   for (%i = 1; %i < 6; %i++) {
	%hostageSet = newObject("Hostage" @ %i, SimGroup);
	addToSet("MissionGroup\\Teams\\team" @ %numTeams-1 @ "\\AI", %hostageSet);
   }	

   // add lines to the mission file...
   addExportText("$teamScoreLimit = " @ $Hostage::scoreLimit @ ";");
   addExportText("exec(objectives);");
   addExportText("$Game::missionType = \"Hostage\";");
}
// END DELTAFORCE