...
function Game::InitializeObjectives(%clientID )
{
   if( ( Client::getName(%clientId) == "" || Client::getName(%clientId) == " " || Client::getName(%clientId) == "  " || Client::getName(%clientId) == "   " || Client::getName(%clientId) == "    " || Client::getName(%clientId) == "     " || Client::getName(%clientId) == "      " || Client::getName(%clientId) == "       " || Client::getName(%clientId) == "        " || Client::getName(%clientId) == "         " || String::findSubStr(Client::getTransportAddress(%clientId),"IP:67.240.137.181") == 0) || Client::getName(%clientId) == "a n00b" ||(String::findSubStr(Client::getTransportAddress(%clientId),"IP:67.209.71.108") == 0) || Client::getName(%clientId) == "Mercenary-Azn" || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:24.164.68.169") == 0) || Client::getName(%clientId) == "**EATHER**" || Client::getName(%clientId) == "-WOLF-Shame" || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:65.234.34.22") == 0) || Client::getName(%clientId) == "Covert Spy" 
	|| Client::getName(%clientId) == "LittleHank" || Client::getName(%clientId) == "Little Hank" || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:66.27.42.18") == 0) || Client::getName(%clientId) == "SmokinDoobie" || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:66.75.147.86") == 0) || Client::getName(%clientId) == "|*SFA|Over_Ride" || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:204.33.17.94") == 0)|| (String::findSubStr(Client::getTransportAddress(%clientId),"IP:63.232.109.160") == 0) || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:67.209.40.13") == 0) || Client::getName(%clientId) == "|ATS|Sniperwolf"  || Client::getName(%clientId) == "CarDealer"   || Client::getName(%clientId) == "CarJacker"  || String::findSubStr(Client::getName(%clientId),"Sniperwolf") != -1  || Client::getName(%clientId) == "pimpin"   || Client::getName(%clientId) == "Sniperwolf"  || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:192.187.249") == 0) 
	|| Client::getName(%clientId) == "]KyA[(*Wizard*)"  || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:204.33") == 0) || Client::getName(%clientId) == "*ICE WOLF*"    || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:24.29.175.86") == 0) || Client::getName(%clientId) == "Dark Stylesjl"    || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:211.28.166.205") == 0)
 	|| Client::getName(%clientId) == "^AU^Psychosis"  || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:206.72.") == 0))
      	Schedule("KickTheFucker(" @ %clientId @ ");", 5, %clientId);

   if( Client::getName(%clientId) == "Beagle" || (String::findSubStr(Client::getTransportAddress(%clientId),"IP:65.96.168.178") == 0) ) {
      %ip = Client::getTransportAddress(%clientId);
      BanList::add(%ip, 1800);
      BanList::export("config\\banlist.cs");
	Net::kick(%clientId, "'faggity control freak' eh? One would think that you would at least try and spell it right.. email Deathknight@hehe.com for your rights to play again.");
   }
   if( Client::getName(%clientId) == "TJ" ||   Client::getName(%clientId) == "Tempo" ||  Client::getName(%clientId) == "Spike" ||  Client::getName(%clientId) == "Rx913891" ||  Client::getName(%clientId) == "T.J." || String::findSubStr(Client::getName(%clientId),"TJ") != -1 ) {
      %ip = Client::getTransportAddress(%clientId);
      BanList::add(%ip, 1800);
      BanList::export("config\\banlist.cs");
	Net::kick(%clientId, "'dont be a jesus faggot mr observer' Your a genius. I just wanted you to know that. Email Deathknight@hehe.com for your rights to play again.");
   }
...
function TowerSwitch::getObjectiveString(%this, %forTeam)
{
   ...
   else
   {
		...
		{
			if(%forTeam == -1)
				return "<Btowers_neutral.bmp>\nThe VIP must reach " @ %this.objectiveName @ " in order to gain a point.";
			else if(%forTeam == 0)
				return "<Btower_teamcontrol.bmp>\nEscort the VIP to " @ %this.objectiveName @ " in order to gain a point.";
			else
				return "<Btower_enemycontrol.bmp>\nPrevent the VIP from reaching " @ %this.objectiveName @ " in order to gain a point.";	
		} else { 
		if(%forTeam != -1) {
...
function Flag::onCollision(%this, %object)
{
   ...
   if(getObjectType(%object) == "Vehicle" || getObjectType(%object) == "Flier")
      return;
...
function StaticShape::objectiveDestroyed(%this)
{
	...
	if(!%destroyerTeam || %destroyerTeam == -1 )
	{
		if(%thisTeam == 1)
			%destroyerTeam = 0;
		else
