...
{	  
   ...
   Player::setItemCount(%clientId,SOCOM,1);
   Player::setItemCount(%clientId,RepairKit,1);
	Player::useItem(%pl,SOCOM);
...
function DMTEAM::teamMissionObjectives(%teamId)
{
   ...
   if((%teamScore == %highScore) && (%highScore == 0))
      Team::setObjective(%teamId, 5, "~All teams tied at 0.");
   else if((%teamScore == %highScore) && (%numHighs > 1)) 
      Team::setObjective(%teamId, 5, "~Your team is Tied for the lead!");
   else	if((%teamScore == %highScore) && (%numHighs == 1))
      Team::setObjective(%teamId, 5, "~Your team is winning.");
   else
      Team::setObjective(%teamId, 5, "~Your team is losing.");
