...
// by Edgecrusher
$InvList[iarmorLib] = 1;
$RemoteInvList[iarmorLib] = 1;
$ArmorType[Male, iarmorLib] = armormLib;
$ArmorType[Female, iarmorLib] = armormLib;
$ArmorName[armormLib] = iarmorLib;
$ArmorName[armorfLib] = iarmorLib;

ItemData iarmorLib
{
	heading = $InvHead[ihMar];
	description = "Marine Librarian";
	className = "Armor";
	price = 70;
};

PlayerData armormLib 
{
	className = "Armor";
	shapeFile = "marine";
...
function armormLib::onPlayerContact(%targetPlayer, %sourcePlayer)
{
//	Drain(%damagedPlayer, %damagingPlayer);
}


function armormLib::onGrenade(%player)
{
...
function armormLib::onBeacon(%player, %item)
{
...
function armormLib::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armormLib::onMine(%player)
{
...
function armorfLib::onPlayerContact(%targetPlayer, %sourcePlayer)
{
//	Drain(%damagedPlayer, %damagingPlayer);
}

function armorfLib::onGrenade(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Haywire Bomb will detonate in 6 seconds");
	%obj = newObject("","Mine","Psibomb");
	Armor::ThrowGrenade(%player, %obj);
...
function armorfLib::onBeacon(%player, %item)
{
	startShield(Player::getClient(%player), %player);
	Player::decItemCount(%player,%item);
}

function armorfLib::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armorfLib::onMine(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Delusion Mine deployed.");
	if(%player.throwTime < getSimTime() )
	{
