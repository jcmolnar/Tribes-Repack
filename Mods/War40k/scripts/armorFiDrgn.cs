//Fire Dragon
// by Edgecrusher
$InvList[iarmorFiDrgn] = 1;
$RemoteInvList[iarmorFiDrgn] = 1;
$ArmorType[Male, iarmorFiDrgn] = armormFiDrgn;
$ArmorType[Female, iarmorFiDrgn] = armorfFiDrgn;
$ArmorName[armormFiDrgn] = iarmorFiDrgn;
$ArmorName[armorfFiDrgn] = iarmorFiDrgn;

ItemData iarmorFiDrgn
{
...
function armormFiDrgn::onPlayerContact(%damagedPlayer, %damagingPlayer)
{
//	Drain(%damagedPlayer, %damagingPlayer);
}

function armormFiDrgn::onGrenade(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Meltabomb will detonate in 12 seconds.");
	%obj = newObject("","Mine","Nukebomb");
	Armor::ThrowGrenade(%player, %obj);
}

function armormFiDrgn::onBeacon(%player, %item)
{
	Client::sendMessage(Player::getClient(%player),1, "Firebomb will explode in 4 seconds. Run.");
...
function armormFiDrgn::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armormFiDrgn::onMine(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Fire Mine deployed.");
	if(%player.throwTime < getSimTime() )
	{
...
function armorfFiDrgn::onPlayerContact(%damagedPlayer, %damagingPlayer)
{
//	Drain(%damagedPlayer, %damagingPlayer);
}

function armorfFiDrgn::onGrenade(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Meltabomb will detonate in 12 seconds.");
	%obj = newObject("","Mine","Nukebomb");
	Armor::ThrowGrenade(%player, %obj);
...
function armorfFiDrgn::onBeacon(%player, %item)
{
	Client::sendMessage(Player::getClient(%player),1, "Firebomb will explode in 6 seconds. Run.");
	if(%player.throwTime < getSimTime() )
	{
		%obj = newObject("","Mine","Firebomb");
		addToSet("MissionCleanup", %obj);
...
function armorfFiDrgn::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armorfFiDrgn::onMine(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Fire Mine deployed.");
	if(%player.throwTime < getSimTime() )
	{
