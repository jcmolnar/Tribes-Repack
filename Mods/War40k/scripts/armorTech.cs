//Techmarine
// by Edgecrusher
$InvList[iarmorTech] = 1;
$RemoteInvList[iarmorTech] = 1;
$ArmorType[Male, iarmorTech] = armormTech;
$ArmorType[Female, iarmorTech] = armormTech;
$ArmorName[armormTech] = iarmorTech;
$ArmorName[armorfTech] = iarmorTech;
...
function armormTech::onPlayerContact(%targetPlayer, %sourcePlayer)
{
	Repair(%targetPlayer, %sourcePlayer);
}

function armormTech::onGrenade(%player)
{
	%obj = newObject("","Mine","Forcegrenade");
	Armor::ThrowGrenade(%player, %obj);
}

function armormTech::onBeacon(%player, %item)
{
	DeployPointDef(Player::getClient(%player), %player, %item);
}

function armormTech::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armormTech::onMine(%player)
{
...
function armorfTech::onPlayerContact(%targetPlayer, %sourcePlayer)
{
	Repair(%targetPlayer, %sourcePlayer);
}

function armorfTech::onGrenade(%player)
{
	%obj = newObject("","Mine","Forcegrenade");
	Armor::ThrowGrenade(%player, %obj);
}

function armorfTech::onBeacon(%player, %item)
{
	DeployPointDef(Player::getClient(%player), %player, %item);
}

function armorfTech::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armorfTech::onMine(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Ark Mine deployed.");
	if(%player.throwTime < getSimTime() )
	{
