...
function use(%desc) 
{
	...
	{
		useItem(%type);
		%Name = GameBase::getDataName(%desc);
		if((%Name == SMRPack) || (%Name == OpticPack))
		{
			useItem(%type);
		}
	}
...
function targetClient() 
{
	...
	{
		if(Client::getTeam(getManagerId()) == Client::getTeam($lastClientMessage)) %cmd = "Escort " @ Client::getName($lastClientMessage) @ ".~wescfr"; else %cmd = "Attack " @ Client::getName($lastClientMessage) @ ".~wattway";
...
{
	if(%id == $centerPrintId) Client::centerPrint("", 0);
} 

function remoteITXT(%manager, %msg) 
{
	if(%manager == 2048) Control::setValue(EnergyDisplayText, %msg);
} 
...
function remoteCP(%manager, %msg, %timeout) 
{
	...
	{
		$centerPrintId++;
		if(%timeout) schedule("clearCenterPrint(" @ $centerPrintId @ ");", %timeout);
...
function showGfxSW()
{
	function Game::EndFrame()
	{
		echo($ConsoleWorld::FrameRate @ " P:" @ $GFXMetrics::EmittedPolys @ ", " @ $GFXMetrics::RenderedPolys @ " TSU:", $GFXMetrics::textureSpaceUsed);
	}
	$Console::LastLineTimeout = 1000;
...
{
	if(%server == 2048) setHudTimer(%time);
} 
...
{ 
	...
	Control::setValue(ProgressText, "<jc><f1>War40k Engine\n<f0>...final preparations..."); 
...
{
	if(!isObject("commandGui")) loadObject("commandGui", "gui\\command.gui");
	...
	if(%player == 2048)
	{
		...
		if (!$pref::userCDOverride)
		{
			...
			else if ($cdTrack != "")
			{
				...
				if ($pref::cdMusic) rbPlay (CD, $cdTrack);
			}
		}
	}
} 
...
function onConnection(%message)
{
	...
	else
	{
		...
		if(%message == "Dropped")
		{
			if($errorString == "") $errorString = "Connection to server lost:\nServer went down.";
			else $errorString = "Connection to server lost:\n" @ $errorString;
			GuiPushDialog(MainWindow, "gui\\MessageDialog.gui");
			schedule("Control::setValue(MessageDialogTextFormat, $errorString);", 0);
		}
...
function setupRecorderFile(%fileName)
{
	...
	{
		if(isFile("recordings\\" @ %fileName)) echo("Warning- " @ %fileName @ " File Already Exists");
