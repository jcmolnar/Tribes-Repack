//DARK REAPER 
// by Edgecrusher
$InvList[iarmorDReaper] = 1;
$RemoteInvList[iarmorDReaper] = 1;
$ArmorType[Male, iarmorDReaper] = armormDReaper;
$ArmorType[Female, iarmorDReaper] = armorfDReaper;
$ArmorName[armormDReaper] = iarmorDReaper;
$ArmorName[armorfDReaper] = iarmorDReaper;

ItemData iarmorDReaper
{
	heading = $InvHead[ihEld];
	description = "Dark Reaper";
	className = "Armor";
	price = 30;
};

PlayerData armormDReaper
{
	className = "Armor";
	shapeFile = "marmor";
...
function armormDReaper::onPlayerContact(%targetPlayer, %sourcePlayer)
{
	Armor::onPlayerContact(%targetPlayer, %sourcePlayer);
}

function armormDReaper::onGrenade(%player)
{
	%obj = newObject("","Mine","Krakgren");
	Armor::ThrowGrenade(%player, %obj);
}

function armormDReaper::onBeacon(%player, %item)
{
	Client::sendMessage(Player::getClient(%player),1, "Fusion Bomb tossed!.");
	if(%player.throwTime < getSimTime() )
	{
		%obj = newObject("","Mine","Fusionbomb");
		addToSet("MissionCleanup", %obj);
		GameBase::throw(%obj, %player, 17,false); //* %clientid.throwStrength
		%player.throwTime = getSimTime() + 0.5;
		Player::decItemCount(%player,%item);
	}
}

function armormDReaper::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armormDReaper::onMine(%player)
{
...
function armorfDReaper::onPlayerContact(%targetPlayer, %sourcePlayer)
{
	Armor::onPlayerContact(%targetPlayer, %sourcePlayer);
}

function armorfDReaper::onGrenade(%player)
{
	%obj = newObject("","Mine","Krakgren");
	Armor::ThrowGrenade(%player, %obj);
}

function armorfDReaper::onBeacon(%player, %item)
{
	Client::sendMessage(Player::getClient(%player),1, "Fusion Bomb Tossed!");
	if(%player.throwTime < getSimTime() )
	{
...
function armorfDReaper::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armorfDReaper::onMine(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Krak Mine deployed.");
	if(%player.throwTime < getSimTime() )
	{
