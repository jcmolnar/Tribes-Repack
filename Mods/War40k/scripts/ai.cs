...
//medium = marmor
//heavy = harmor
$AI::defaultArmorType = "armormMercenary";
...
//if there is no key then they don't exist yet
	if(%key == "")
	{
		...
		if( %aiFound == 0 ) dbecho(1, "No drones exist...");
		else dbecho(1, %aiFound @ " drones installed..." );
	}
//respawning dead AI with original name and path
	else     
	{
...
function AI::setWeapons(%aiName)
{
	...
	%team = (client::getteam(%aiId)+1)%2; //Team Specific Races
	...
	{	
		...
		{
			buyItem(%aiId,%item);
			if(%item.className == Weapon) %aiId.spawnWeapon = %item;
		}
		%aiId.spawn= "";
		if(%aiId.spawnWeapon != "") 
		{
			Player::useItem(%pl,%aiId.spawnWeapon);
			%aiId.spawnWeapon="";
		}
		Player::decItemCount(%aiId,RepairPack,1);
		Player::setItemCount(%aiId,DeployableInvPack,1);
	}
	else
	{
	...
	Player::mountItem(%aiId, %aiId.spawnWeapon, 0);
...
function ai::periodicWeaponChange(%aiName)
{
	...
	else
	{
		...
		{	
			Player::mountItem(%aiId, Bolter, 0);
...
//check for internal data
		...
		else dbecho(1, "no info to spawn ai...");
	}
}
