...
function TourneyCheckEnd()
{
	for(%i = 0; %i < getNumTeams(); %i++) %teamcount[%i] = 0;
	...
	if(%teamplayers < 2) 
	{
		...
		for(%i = 0; %i < getNumTeams(); %i++)
		{
			if(%teamcount[%i] > 0) $teamScore[%i] += 500;
		}
...
function ObjectiveMission::setObjectiveHeading()
{
	if($missionComplete)
	{
		...
		if(%tieGame)
		{
			for(%g = 0; %g <= %tie; %g++)
			{
				...
				if(%g == %tie-1) %names = %names @ " and "; 
				else if(%g != %tie) %names = %names @ ", "; 
			}
			if(%tie > 1) %names = %names @ " all"; 
		}
		for(%i = -1; %i < getNumTeams(); %i++)
		{
			...
			if(!%tieGame)
			{
				if(%i == %curLeader)
				{
					if($teamScore[%curLeader] == 1) Team::setObjective(%i, 1, "<F5>Your team won the mission with " @ $teamScore[%curLeader] @ " point!");
					else Team::setObjective(%i, 1, "<F5>Your team won the mission with " @ $teamScore[%curLeader] @ " points!");
				}
				else
				{
					if($teamScore[%curLeader] == 1) Team::setObjective(%i, 1, "<F5>The " @ getTeamName(%curLeader) @ " team won the mission with " @ $teamScore[%curLeader] @ " point!");
					else Team::setObjective(%i, 1, "<F5>The " @ getTeamName(%curLeader) @ " team won the mission with " @ $teamScore[%curLeader] @ " points!");
				}
			}
			else
			{
				...
				else Team::setObjective(%i, 1, "<F5>The mission ended in a tie where each team had a score of " @ $teamScore[%curLeader]);
			}
	...
	if(!$Server::timeLimit) %str = "<f1>- No time limit on the game.";
	else if($timeLimitReached) %str = "<f1>- Time limit reached.";
	...
	else %str = "<f1>- Time remaining: " @ floor($Server::timeLimit - (getSimTime() - $missionStartTime) / 60) @ " minutes.";
	for(%i = -1; %i < getNumTeams(); %i++)
	{
...
function objective::displayBitmap(%team, %line)
{
	...
	if(%bitmap1 == "" || %bitmap2 == "") Team::setObjective(%team, %line, " ");
	else Team::setObjective(%team, %line, "<jc><B0,0:" @ %bitmap1 @ "><B0,0:" @ %bitmap2 @ ">");
}
...
// just reschedule the check for a minute hence
	...
	if(%curTimeLeft <= 0 && $matchStarted)
	{
		...
		for(%i = 0; (%obj = Group::getObject(%set, %i)) != -1; %i++) GameBase::virtual(%obj, "timeLimitReached", %clientId);
		ObjectiveMission::missionComplete();
	}
	else
	{
		if(%curTimeLeft >= 20) schedule("Game::checkTimeLimit();", 20);
		else schedule("Game::checkTimeLimit();", %curTimeLeft + 1);
...
function ObjectiveMission::checkScoreLimit()
{
	...
	if(%done) ObjectiveMission::missionComplete();
}

function ObjectiveMission::checkPoints()
{
	for(%i = 0; %i < getNumTeams(); %i++) $teamScore[%i] += $deltaTeamScore[%i] / 12;
...
function ObjectiveMission::initCheck(%object)
{
	if($TestMissionType == "")
	{
		...
		if(%name == Flag)
		{
			if(gamebase::getteam(%object) != -1) $TestMissionType = "CTF";
			else $TestMissionType = "F&R";
		}
		else if(%object.objectiveName != "" && %object.scoreValue) $TestMissionType = "D&D";
		else if(%name == TowerSwitch) $NumTowerSwitchs++;
	}
	%object.trainingObjectiveComplete = "";
	%object.objectiveLine = "";
	if(GameBase::virtual(%object, objectiveInit)) addToSet("MissionCleanup/ObjectivesSet", %object);
}
...
function ObjectiveMission::refreshTeamScores()
{
	...
	for(%i = -1; %i < %nt; %i++)
	{
		...
		for(%j = 0; %j < %nt; %j++) Team::setObjective(%i,%j+$firstTeamLine, "<f1>- Team " @ getTeamName(%j) @ " score = " @ $teamScore[%j]);
	}
}
...
function ObjectiveMission::objectiveChanged(%this)
{
	...
	{
		for(%i = -1; %i < getNumTeams(); %i++) Team::setObjective(%i,%this.objectiveLine, "<f1> " @ GameBase::virtual(%this, getObjectiveString, %i));
	}
}
...
function Mission::init()
{
	...
	$Deathmatch = $numTeams == 1;
	$teamplay = !$Deathmatch;
	$firstTeamLine = 7;
	$firstObjectiveLine = $firstTeamLine + $numTeams + 1;
	for(%i = -1; %i < $numTeams; %i++)
	{
		...
		for(%j = 0; (%dropPoint = Group::getObject(%dropSet, %j)) != -1; %j++) addToSet("MissionCleanup/TeamDrops" @ %i, %dropPoint);
	}
	$numObjectives = 0;
	...
	if($TestMissionType == "")
	{
		if($NumTowerSwitchs) $TestMissionType = "C&H";
		else $TestMissionType = "NONE";
		$NumTowerSwitchs = "";
	}
	AI::setupAI();
}
...
function Client::leaveGame(%clientId)
{
	...
	for(%i = 0; (%obj = Group::getObject(%set, %i)) != -1; %i++) GameBase::virtual(%obj, "clientDropped", %clientId);
}

function Game::clientKilled(%playerId, %killerId)
{
	%set = nameToID("MissionCleanup/ObjectivesSet");
	for(%i = 0; (%obj = Group::getObject(%set, %i)) != -1; %i++) GameBase::virtual(%obj, "clientKilled", %playerId, %killerId);
}
...
function Player::enterMissionArea(%this)
{
	%set = nameToID("MissionCleanup/ObjectivesSet");
	%this.outArea = "";
	for(%i = 0; (%obj = Group::getObject(%set, %i)) != -1; %i++) GameBase::virtual(%obj, "playerEnterMissionArea", %this);
	Client::sendMessage(Player::getClient(%this),1,"You have re-entered the mission area.");
}
...
function alertPlayer(%player, %count)
{
	if(%player.outArea == 1)
	{
		...
		{
			%set = nameToID("MissionCleanup/ObjectivesSet");
			for(%i = 0; (%obj = Group::getObject(%set, %i)) != -1; %i++) GameBase::virtual(%obj, "playerLeaveMissionArea", %player);
		}
	}
}
...
//echo("switch collision ", %object);
	...
	if(%dropPoints != -1)
	{
		...
		{
			if(%oldDropSet != -1) removeFromSet(%oldDropSet, %dropPoint);
	...
	if(%this.objectiveLine)
	{
		...
		if(%oldTeam != -1) TeamMessages(1, %oldTeam, "The " @ getTeamName(%playerTeam) @ " team has taken your objective.~wLostTower.wav");
...
{
//give player 15 points for capturing the objective
	if(%this.numSwitchTeams == %numChange)
	{
		%client.score+=15;
		Game::refreshClientScore(%client);
		Client::sendMessage(%client, 0, "You receive 15 points for holding the objective!");
	}
}
...
function Flag::getObjectiveString(%this, %forTeam)
{
	...
	if($missionComplete)
	{
		if(%thisTeam == -1)
		{
			if(%this.holdingTeam == %forTeam && %forTeam != -1) return "<Bflag_atbase.bmp>\nYour team finished the mission in control of " @ %this.objectiveName @ ".";
			else if(%this.holdingTeam == -1) return "<Bflag_neutral.bmp>\nNo team finished the mission in control of " @ %this.objectiveName @ ".";
			else
			{
				if(%forTeam != -1) return "<Bflag_enemycaptured.bmp>\nThe " @ getTeamName(%this.holdingTeam) @ " team finished the mission in control of " @ %this.objectiveName @ ".";
				else return "<Bflag_atbase.bmp>\nThe " @ getTeamName(%this.holdingTeam) @ " team finished the mission in control of " @ %this.objectiveName @ ".";
			}
		}
		else if(%forTeam != -1)
		{
			if(%thisTeam == %forTeam) return "<Bflag_atbase.bmp>\nYour fuel was captured " @ %this.enemyCaps @ " times.";
			else return "<Bflag_enemycaptured.bmp>\nYour team captured the " @ getTeamName(%thisTeam) @ " fuel " @ %this.caps[%forTeam] @ " times.";
		}
		else return "<Bflag_atbase.bmp>\nThe " @ getTeamName(%thisTeam) @ "'s fuel was captured " @ %this.enemyCaps @ " times.";
	}
	else
	{
		if(%thisTeam == -1)
		{
			if(%forTeam != -1)
			{
				if(%this.holdingTeam == %forTeam) return "<Bflag_atbase.bmp>\nDefend " @ %this.objectiveName @ ".";
				else if(%this.holdingTeam != -1) return "<Bflag_enemycaptured.bmp>\nGrab " @ %this.objectiveName @ " from the " @ getTeamName(%this.holdingTeam) @ " team.";
				else if(%this.carrier != -1)
				{
					if(GameBase::getTeam(%this.carrier) == %forTeam) return "<Bflag_atbase.bmp>\nConvey " @ %this.objectiveName @ " to an empty fuel stand. (carried by " @ Client::getName(Player::getClient(%this.carrier)) @ ")";
					else return "<Bflag_enemycaptured.bmp>\nWaylay " @ Client::getName(Player::getClient(%this.carrier)) @ " and convey " @ %this.objectiveName @ " to your base.";
				}
				else if(%this.atHome) return "<Bflag_neutral.bmp>\nGrab " @ %this.objectiveName @ " and convey it to an empty flag stand.";
				else return "<Bflag_notatbase.bmp>\nFind " @ %this.objectiveName @ " and convey it to an empty fuel stand.";
			}
			else
			{
				if(%this.holdingTeam != -1) return "<Bflag_atbase.bmp>\nThe " @ getTeamName(%this.holdingTeam) @ " team has " @ %this.objectiveName @ ".";
				else if(%this.carrier != -1) return "<Bflag_atbase.bmp>\n" @ Client::getName(Player::getClient(%this.carrier)) @ " has " @ %this.objectiveName @ ".";
		...
		else
		{
			if(%thisTeam == %forTeam)
			{
				if(%this.atHome) return "<Bflag_atbase.bmp>\nDefend your fuel to prevent enemy captures.";
				else if(%this.carrier != -1) return "<Bflag_enemycaptured.bmp>\nReturn your fuel to base. (carried by " @ Client::getName(Player::getClient(%this.carrier)) @ ")";
				else return "<Bflag_notatbase.bmp>\nReturn your fuel to base. (dropped in the field)";
			}
			else
			{
				if(%forTeam != -1)
				{
					if(%this.atHome) return "<Bflag_enemycaptured.bmp>\nGrab the " @ getTeamName(%thisTeam) @ " fuel and touch it to your's to score " @ %this.scoreValue @ " points.";
					else if(%this.carrier == -1) return "<Bflag_notatbase.bmp>\nFind the " @ getTeamName(%thisTeam) @ " fuel and touch it to your's to score " @ %this.scoreValue @ " points.";
					else if(GameBase::getTeam(%this.carrier) == %forTeam) return "<Bflag_atbase.bmp>\nEscort friendly carrier " @ Client::getName(Player::getClient(%this.carrier)) @ " to base.";
					else return "<Bflag_enemycaptured.bmp>\nWaylay enemy carrier " @ Client::getName(Player::getClient(%this.carrier)) @ " and steal his fuel.";
				}
				else
				{
					if(%this.atHome) return "<Bflag_atbase.bmp>\nThe " @ getTeamName(%thisTeam) @ " fuel is at their base.";
					else if(%this.carrier == -1) return "<Bflag_notatbase.bmp>\nThe " @ getTeamName(%thisTeam) @ " fuel has been dropped in the field.";
					else return "<Bflag_atbase.bmp>\n" @ Client::getName(Player::getClient(%this.carrier)) @ " has the " @ getTeamName(%thisTeam) @ " fuel.";
				}
			}
		}
	}
...
function Flag::onDrop(%player, %type)
{
	...
	{
		MessageAllExcept(%playerClient, 0, %dropClientName @ " dropped the " @ getTeamName(%flagTeam) @ " fuel!");
		Client::sendMessage(%playerClient, 0, "You dropped the " @ getTeamName(%flagTeam) @ " fuel!");
		TeamMessages(1, %flagTeam, "Your fuel was dropped in the field.", -2, "", "The " @ getTeamName(%flagTeam) @ " fuel was dropped in the field.");
	}
		...
		else
		{
			...
			if(%flagTeam == -1)
			{
				...
				else
				{
					...
					TeamMessages(0, %holdTeam, "Your fuel was returned to base.~wflagreturn.wav", -2, "", "The " @ getTeamName(GameBase::getTeam(%flag.flagStand)) @ " fuel was returned to base.~wflagreturn.wav");
			...
			{
				TeamMessages(0, %flagTeam, "Your fuel was returned to base.~wflagreturn.wav", -2, "", "The " @ getTeamName(%flagTeam) @ " fuel was returned to base.~wflagreturn.wav");
				GameBase::setPosition(%flag, %flag.originalPosition);
				Item::setVelocity(%flag, "0 0 0");
			}
			%flag.atHome = true;
...
{
//So Warp Spiders cant cap flag
if(Player::getMountedItem(%object, $BackpackSlot) == Warpxpack) return;	       
	if(getObjectType(%object) != "Player") return;
// spurious collision
	if(%this.carrier != -1) return;
...
// the flag isn't home! so return it.
			...
			%this.atHome = true;
			MessageAllExcept(%playerClient, 0, %touchClientName @ " returned the " @ getTeamName(%playerTeam) @ " fuel!~wflagreturn.wav");
			Client::sendMessage(%playerClient, 0, "You returned the " @ getTeamName(%playerTeam) @ " fuel!~wflagreturn.wav");
			teamMessages(1, %playerTeam, "Your fuel was returned to base.", -2, "", "The " @ getTeamName(%playerTeam) @ " fuel was returned to base.");
			%this.pickupSequence++;
			ObjectiveMission::ObjectiveChanged(%this);
//score for returning flag
                        %playerClient.score += 5;
			Game::refreshClientScore(%playerClient);
			messageAll(0, Client::getName(%playerClient) @ " receives 5 point return bonus.");
//end return flag scoring
		}
		else
		{
...
// can't cap the neutral flags, duh
				...
				if(%enemyTeam != -1)
				{
					MessageAllExcept(%playerClient, 0, %touchClientName @ " captured the " @ getTeamName(%enemyTeam) @ " fuel!~wflagcapture.wav");
					Client::sendMessage(%playerClient, 0, "You captured the " @ getTeamName(%enemyTeam) @ " fuel!~wflagcapture.wav");
					TeamMessages(1, %playerTeam, "Your team captured the fuel.", %flagTeam, "Your team's fuel was captured.");
					...
					ObjectiveMission::checkScoreLimit();
//fuel carrier gets 25 points for caputure
					%playerClient.score += 25;
					Game::refreshClientScore(%playerClient);
					messageAll(0, Client::getName(%playerClient) @ " receives 25 point capture bonus.");
				}
			}
		}
	}
	...
	{
// it's an enemy's fuel! woohoo!
		if(%object.carryFlag == "")
		{
			if(%object.outArea == "") 
			{
// don't pick up our fuel
				if(%this.holdingTeam == %playerTeam) return;
				...
				if(%flagTeam != -1)
				{
					MessageAllExcept(%playerClient, 0, %touchClientName @ " took the " @ getTeamName(%flagTeam) @ " fuel! ~wflag1.wav");
					Client::sendMessage(%playerClient, 0, "You took the " @ getTeamName(%flagTeam) @ " fuel! ~wflag1.wav");
					TeamMessages(1, %playerTeam, "Your team has the " @ getTeamName(%flagTeam) @ " fuel.", %flagTeam, "Your team's fuel has been taken.");
				}
				else
				{
			...
			Client::sendMessage(%playerClient, 1, "Fuel not in mission area.");
		}
	}
}
...
{
	if(%success) setCommandStatus(%client, 0, "Objective completed.~wobjcomp");
	else setCommandStatus(%client, 0, "Objective failed.");
}

function Flag::setWaypoint(%client, %flag)
{
	if(!%client.autoWaypoint) return;
	%flagTeam = GameBase::getTeam(%flag);
...
function FlagStand::onCollision(%this, %object)
{
	...
	%playerTeam = GameBase::getTeam(%object);
	if(%standTeam == -1 || getObjectType(%object) != "Player" || %object.carryFlag == "" || %playerTeam != %standTeam || %this.flag != "" || GameBase::getTeam(%object.carryFlag) != -1) return;
...
function Flag::clientKilled(%this, %playerId, %killerId)
{
	...
	if(%player == -1 || %killer == -1) return;
	%flagTeam = GameBase::getTeam(%this);
	if(%flagTeam == -1) return;
	%playerTeam = GameBase::getTeam(%player);
	%killerTeam = GameBase::getTeam(%killer);
	if(%playerTeam == %killerTeam) return;
...
function Flag::clientDropped(%this, %clientId)
{
	...
	if(%type != -1) Player::dropItem(%clientId, %type);
}
...
// if a guy leaves the area, warp the flag back to its base
	if(%this.carrier == %playerId)
	{
		...
		else
		{
			...
			MessageAllExcept(%playerClient, 0, %clientName @ " left the mission area while carrying the " @ getTeamName(%team) @ " fuel!");
			Client::sendMessage(%playerClient, 0, "You left the mission area while carrying the " @ getTeamName(%team) @ " fuel!");
			TeamMessages(1, %team, "Your fuel was returned to base.~wflagreturn.wav", -2, "", "The " @ getTeamName(%team) @ " fuel was returned to base.");
...
//		if(%type.className == Turret)
//		{
//			%killpoints = (floor(%type.maxdamage * 3));
//			if(%killpoints < 1) %killpoints = 1;
//		}
//		else
//		{
		%killpoints = (floor(%type.maxdamage / 2));
		if(%killpoints < 1) %killpoints = 1;
//		}
...
// test if it's really an objective
		...
		if(%thisTeam == %destroyerTeam)
		{
			if(%playerClient != -1)
			{
				if (GameBase::getDataName(%this).mapFilter != -1)
				{
					%playerClient.score = %playerClient.score - (%killpoints * 5);
					bottomprint(%playerClient, "<f0>Score:<f1> -" @ (%killpoints * 5));
					Game::refreshClientScore(%playerClient);
				}
			}
