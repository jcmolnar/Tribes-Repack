//GUARDIAN
// by Edgecrusher
$InvList[iarmorGuardian] = 1;
$RemoteInvList[iarmorGuardian] = 1;
$ArmorType[Male, iarmorGuardian] = armormGuardian;
$ArmorType[Female, iarmorGuardian] = armorfGuardian;
$ArmorName[armormGuardian] = iarmorGuardian;
$ArmorName[armorfGuardian] = iarmorGuardian;


ItemData iarmorGuardian
{
...
function armormGuardian::onPlayerContact(%targetPlayer, %sourcePlayer)
{
	Armor::onPlayerContact(%targetPlayer, %sourcePlayer);
}

function armormGuardian::onGrenade(%player)
{
	%obj = newObject("","Mine","Shockgrenade");
	Armor::ThrowGrenade(%player, %obj);
}

function armormGuardian::onBeacon(%player, %item)
{
	Armor::SpeedBooster(%player, %item, 400);
}

function armormGuardian::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armormGuardian::onMine(%player)
{
...
function armorfGuardian::onPlayerContact(%targetPlayer, %sourcePlayer)
{
	Armor::onPlayerContact(%targetPlayer, %sourcePlayer);
}

function armorfGuardian::onGrenade(%player)
{
	%obj = newObject("","Mine","Shockgrenade");
	Armor::ThrowGrenade(%player, %obj);
}

function armorfGuardian::onBeacon(%player, %item)
{
	Armor::SpeedBooster(%player, %item, 400);
}

function armorfGuardian::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armorfGuardian::onMine(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Mine deployed.");
	if(%player.throwTime < getSimTime() )
	{
