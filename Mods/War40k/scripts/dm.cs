...
function Game::clientKilled(%playerId, %killerId) 
{
	if($teamplay) 
	{
		...
		if(%kteam == %pteam) $teamScore[%kteam]--;
		else $teamScore[%kteam]++;
		DMTEAM::checkMissionObjectives();
	}
	else DM::checkMissionObjectives(%killerId);
}
...
function Player::checkLMATimeout(%player, %seqCount) 
{
	...
	if(%player.dieSeqCount == %seqCount) remoteKill(Player::getClient(%player));
}
...
function alertPlayer(%player, %count) 
{
	if(%player.outArea == 1) 
	{
		...
		if(%count > 1) schedule("alertPlayer(" @ %player @ ", " @ %count - 1 @ ");",1.5,%clientId);
		else schedule("leaveMissionAreaDamage(" @ %clientId @ ");",1,%clientId);
	}
}
...
{
	if(DM::missionObjectives(%playerId)) schedule("nextMission();", 0);
	if($DMScoreLimit > 0) 
	{
...
function DM::missionObjectives() 
{
	...
	for (%i = 0; %i < %numClients; %i++) %clientList[%i] = getClientByIndex(%i);
	%doIt = 1;
	while(%doIt == 1)
	{
		%doIt = "";
	...
	for (%l = -1; %l < 1; %l++)
	{
		...
		for (%s = %lineNum+1; %s < 30; %s++) Team::setObjective(%l, %s, " ");
	}
	$timeReached="";
}
...
function DMTEAM::echoScores() 
{
	...
	for(%i = 1; %i < $numTeams; %i++) %score = %score @ ", " @ getTeamName(%i) @ ": " @ $teamScore[%i];
	MessageAll(0, %score);
...
{
	for(%p = 0; %p < $numTeams; %p++)
	{
		if (DMTEAM::teamMissionObjectives(%p)) schedule("Server::nextMission();", 0);
	}
}
...
function DMTEAM::teamMissionObjectives(%teamId) 
{
	...
	{
		for(%r = 0; %r < $numTeams; %r++) 
		{
			if(%teamScore == %highScore) Team::setObjective(%teamId, 2, "~f0Your team is victorious!");
			else if((%teamScore == %highScore) && (%numHighs > 1)) Team::setObjective(%teamId, 2, "~f0Your team ended up tied for the lead!");
			else Team::setObjective(%teamId, 2, "~f0Your team lost!");
		}
		Team::setObjective(%teamId, 3, "\n");
		%lnum = 4;
		for(%q = 0; %q < $numTeams; %q++) 
		{
	...
	if((%teamScore == %highScore) && (%highScore == 0)) Team::setObjective(%teamId, 5, "~f0All teams tied at 0.");
	else if((%teamScore == %highScore) && (%numHighs > 1)) Team::setObjective(%teamId, 5, "~f0Your team is Tied for the lead!");
	else if((%teamScore == %highScore) && (%numHighs == 1)) Team::setObjective(%teamId, 5, "~f0Your team is winning.");
	else Team::setObjective(%teamId, 5, "~f0Your team is losing.");
...
function Game::refreshClientScore(%clientId) 
{
	...
	if($teamplay) Client::setScore(%clientId, "%n\t%t\t" @ %clientId.score @ "\t%p\t%l", %clientId.score);
	else Client::setScore(%clientId, "%n\t " @ %clientId.scoreKills @ "\t" @ %clientId.scoreDeaths @ "\t" @ %clientId.ratio @ ".0%\t%p\t %l", %clientId.ratio);
	DM::missionObjectives();
}
...
function Mission::init() 
{
	...
	$teamplay = !$Deathmatch;
	for(%i = 0; %i < $numTeams; %i++) $teamScore[%i] = 0;
	if (!$Deathmatch) 
	{
