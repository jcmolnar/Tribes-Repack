//Eldar Scout
// by Edgecrusher
$InvList[iarmorRanger] = 1;
$RemoteInvList[iarmorRanger] = 1;
$ArmorType[Male, iarmorRanger] = armormRanger;
$ArmorType[Female, iarmorRanger] = armorfRanger;
$ArmorName[armormRanger] = iarmorRanger;
$ArmorName[armorfRanger] = iarmorRanger;

ItemData iarmorRanger
{
...
function armormRanger::onPlayerContact(%targetPlayer, %sourcePlayer)
{
//	Steal(%targetPlayer, %sourcePlayer);
}

function armormRanger::onGrenade(%player)
{
	%obj = newObject("","Mine","Concussion");
	Armor::ThrowGrenade(%player, %obj);
}

function armormRanger::onBeacon(%player, %item)
{
	DeploySatchel(Player::getClient(%player), %player, %item);
}

function armormRanger::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armormRanger::onMine(%player)
{
...
function armorfRanger::onPlayerContact(%targetPlayer, %sourcePlayer)
{
//	Steal(%targetPlayer, %sourcePlayer);
}

function armorfRanger::onGrenade(%player)
{
	%obj = newObject("","Mine","Concussion");
	Armor::ThrowGrenade(%player, %obj);
}

function armorfRanger::onBeacon(%player, %item)
{
	DeploySatchel(Player::getClient(%player), %player, %item);
}

function armorfRanger::onRepairKit(%player)
{
	Armor::onRepairKit(%player);
}

function armorfRanger::onMine(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Mine deployed.");
	if(%player.throwTime < getSimTime() )
	{
