...
function buy(%desc, %bulk) {
	remoteEval(2048, buyItem, %desc, %bulk);
}
function sell(%desc, %bulk) {
	remoteEval(2048, sellItem, %desc, %bulk);
}
function use(%desc) {
	remoteEval(2048, useItem, %desc);
}
function drop(%desc, %bulk) {
	remoteEval(2048,dropItem, %desc, %bulk);
}

function throwStart() { }

function throwRelease(%desc) { return false; }
...
function remoteITXT(%manager, %txt) {
   if(%manager == 2048)
      schedule("Control::setValue(\"EnergyDisplayText\", \""@%txt@"\");", 0.1);
}
...
{
   if(%pctDone < 0.1)
      %text = "Initializing Spells...";
   else if(%pctDone < 0.2)
      %text = "Earth, Ice, Wind, Lightning, Water...";
   else if(%pctDone < 0.3)
      %text = "Initializing Items...";
   else if(%pctDone < 0.4)
      %text = "Armor, Weapons, Items...";
   else if(%pctDone < 0.5)
      %text = "Gathering Map...";
   else if(%pctDone < 0.6)
      %text = "Gathering character info...";
   else if(%pctDone < 0.7)
      %text = "Gathering Quest log...";
   else if(%pctDone < 0.8)
      %text = "Downloading...";
   else
      %text = "Loading RPG world...";

   //Control::setText(ProgressText, "Loading " @ %blockName @ " data...");
   Control::setValue(ProgressText, "<jc><f1>" @ %text);
  // Control::setValue(ProgressText, "<jc><f1>" @ %blockName);
   Control::setValue(ProgressSlider, %pctDone * 0.95);
}
...
function dataFinished()
{
   ...
   // finished transfer.
	...
	//	rbStop (CD);
	//}
   Control::setValue(ProgressText, "<jc><f0>Get ready to enter...");
   Control::setValue(ProgressSlider, 0.95);
   ...
   echo("Flushing Texture Cache");
   flushTextureCache();
   Control::setValue(ProgressSlider, 0.96);
}
...
function remotedropclient(%manager) {
	if(%manager == 2048) {
		DropClient(); //EndGame();
	}
}
...
$pref::requestTimeoutTime = 10;

function rpgfetchdata() { return False; }//RM doesn't support this

////////////////////////////////////////////////////////////
//Do not edit
$__RM__CLIENT_CS       = 0x0cf;
