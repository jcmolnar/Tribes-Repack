...
$deathMsg[$MortarDamageType, 0]      = "%1 funks %2 into oblivion.";
$deathMsg[$MortarDamageType, 1]      = "%2 didn't see that last gas attack from %1.";
...
$deathMsg[$BlasterDamageType, 1]     = "%2 succumbs to %1's rain of cherries.";
...
$deathMsg[$ElectricityDamageType, 0] = "%2 Gets %4 soul pulled out by %1.";
$deathMsg[$ElectricityDamageType, 1] = "%1 pulled a rabbit out of %2's head.";
$deathMsg[$ElectricityDamageType, 2] = "%1 sent %2's soul to Odin.";
$deathMsg[$ElectricityDamageType, 3] = "%2 dies for %1's sins.";
...
$deathMsg[$MissileDamageType, 0]	    = "%2 gets a hot rocket injection from %1.";
$deathMsg[$MissileDamageType, 1]	    = "%1 gives %2 a lesson in rocketry.";
$deathMsg[$MissileDamageType, 2]	    = "%2 polishes %1's rocket.";
$deathMsg[$MissileDamageType, 3]	    = "%2 rides %1's big one.";
...
$deathMsg[-2,0]						 = "%1 visits Valhalla.";
$deathMsg[-2,1]						 = "%1 kicks %2 own ass.";
...
$spawnBuyList[1] = EnergyRifle;
$spawnBuyList[2] = Rocklauncher;
$spawnBuyList[3] = Disclauncher;
$spawnBuyList[4] = DiscAmmo;
$spawnBuyList[5] = RepairKit;
$spawnBuyList[6] = "TurretPack";
$spawnBuyList[7] = "beacon";
$spawnBuyList[8] = "MineAmmo";
$spawnBuyList[9] = "Grenade";
$spawnBuyList[10] = "blaster";
$spawnBuyList[11] = "";
...
function remoteCommandMode(%clientId)
{
   ...
   // or tied, possessed, or possessing
	%player = Client::getOwnedObject(%clientId);
	if(%player.frozen) return;

   if(!%clientId.guiLock && !%clientId.poss && !%clientId.possy && !%clientId.tied)
   {
...
function remoteToggleCommandMode(%clientId)
{
	%player = Client::getOwnedObject(%clientId);
	if($treed[%player]){
		Turret::Jump(Client::getControlObject(%clientId), 10); 
		return;
		}
	if(%player.frozen) return;
...
function remoteToggleInventoryMode(%clientId)
{
	...
	if (Client::getGuiMode(%clientId) != $GuiModeInventory){
		...
		if (%clientId.spawnGrace)QuickInv(%clientId);
		if (%clientId.InvAcc)QuickInv(%clientId);
//		if (Player::getItemCount(%player,InvPatch)){
//			Client::sendMessage(%clientId,0,"Attempting inventory patch.");
//			schedule("uplink(" @ %clientId @ ");", 2);
//		}
		...
		%clientId.InvAcc = false;
		if (%clientId.InvPatch){
			Client::sendMessage(%clientId,0,"Inventory patch depleted");
			%clientId.InvPatch = false;
		}
...
function uplink (%clientId)
{
	Quickinv(%clientId);
//	Client::sendMessage(%clientId,1,"Uplink acheived.");
	Player::decItemCount(%clientId,InvPatch);
	%clientId.InvAcc = true;
	%clientId.InvPatch = true;
}
...
function UpdateClientTimes(%time)
{
	...
	%sex = (%time % 220);
		if (%sex > 55 && %sex <65) {
		messageAll(1, "Sex mod Dev. team is looking for skilled programmers, send email to ziptiezmail@netscape.net");
		//echo("send me money, n3ppvw@home.com I will accept all forms.");		
		}
		if (%sex > 15 && %sex <25) {
		messageAll(1, "plasmatica.cjb.net for More information about SEX mod.");
		//echo("If you're female, visit me.. ;)");
		}

	//echo("check: time= "@%time@", sex= "@%sex);
...
{
   messageAll(0, "SEX in " @ %time @ " seconds.");
...
function Game::startMatch()
{
   ...
   messageAll(1, "Let the SEX begin");
...
function Game::playerSpawn(%clientId, %respawn)
{
   if(!$ghosting)
	...
	{	
		...
		%random = (%rnd + 10);
		schedule("penis(" @ %clientId @ ");", %random);
	}
//echo(%clientId);
   ...
   %spawnMarker = Game::pickPlayerSpawn(%clientId, %respawn);
	...
	if(%spawnMarker) {   
 	...
 	if ($build) Player::setItemCount(%clientId, soulsucker, 1);
//==================  respawn Invulnerable
	$Server::respawnInvulnerableTime = 10;
	   if(%respawn){
	%clientId.InvAcc = true;
	%clientId.spawnGrace = true;
	$Server::respawnInvulnerableTime = 5;
	schedule("SpawnInvTime(" @ %clientId @ ");", 5);
		}

         if($Server::respawnInvulnerableTime)
         {
		GameBase::startFadeout(%pl); 
            %damage = %armor.maxDamage - 0.001;
            GameBase::setEnergy(%pl, 10);
            GameBase::setDamageLevel(%pl, %damage);
            GameBase::setAutoRepairRate(%pl, %damage / $Server::respawnInvulnerableTime);
            %pl.invulnerable = true;
            for(%i=0; %i < 6.28; %i += 1.256) {
			%trans = Vector::add(getBoxCenter(%pl), Vector::getFromRot("0 0 " @ %i, 1.3, 1.2));
			Projectile::spawnProjectile("ArmChan", "0 0 1 0 0 0 0 0 1 " @ %trans, %pl, "0 0 -1.5");
		}
            schedule("incInvulnerable(" @ %pl @ ", 0);", 0, %pl);
		      ...
		      if(%weapon != -1) {
				...
				bottomprint(%clientId,%clientId.SEXclanmessage,0);
		      }
         }
// ================================================
	   if(!%respawn)
   {
      ...
      bottomprint(%clientId, "<jc>Welcome to <f2>SEX<f1> Mod. By <f2>Plasmatic\nziptiezmail@netscape.net\n<f0>Mission: <f1>" @ $missionName @ "   <f0>Mission Type: <f1>" @ $Game::missionType @ "\n<f0>Press <f1>'O'<f0> for specific objectives.",15);

	schedule("SpawnInvTime(" @ %clientId @ ");", 15);
	schedule("remoteToggleInventoryMode("@%clientId@");",1);
   }
      ...
      if (%clientId.quickinv) schedule("remoteToggleInventoryMode("@%clientId@");",1);   
      return true;
	}
	else {
		Client::sendMessage(%clientId,0,"Sorry, not enough respawn markers in map. Try again in a sec.");
      return false;
	}
}
...
{
   if(%time > $Server::respawnInvulnerableTime)
   {
      %player.invulnerable = "";
...
{echo(Client::getTeam(%client));
	if(%client.dead == 1 && Client::getTeam(%client) != -1)
   ...
   else 
   {
	if($personalskins)
      ...
      if(%clientId.justConnected)
      {
         ...
         centerprint(%clientId, $SEXy@$Server::JoinMOTD, 0);
...
function Game::CheckTourneyMatchStart()
{
   ...
   {
      if(%notReadyCount == 1)
         MessageAll(0, Client::getName(%notReady[0]) @ " is scared of SEX!");
...
// added versitility to function -Plasmatic
...
{
%cl = GameBase::getControlClient(%killerId);

// Plasmaticrocks, AGAIN!!! Hack to give players kill points for optical rocket.
// reworked, not used now -Plas

 if (%damageType == 12 && %killerId.oprocketowner){
...
{
   echo("Player " @ %player @ " entered the ORGY!!!");
}

function Player::leaveMissionArea(%player)
{
   echo("Player " @ %player @ " Had enough SEX");
}
