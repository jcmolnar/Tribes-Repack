...
{
   remoteEval(2048, fetchdata, %password);
}
...
function remoteMODInfo(%server, %modString)
{
   if(%server == 2048)
   {
      ...
      if($ServerModInfo != "")
      {
         Control::setValue(ModTextString, "<jc><f1>This server has been modified for maximum satisfaction.\n<f0>" @ %modString);
      }
   }
}
...
{
   if(%pctDone < 0.1)
      %text = "Initializing Personal SEX wax Assistant...";
   else if(%pctDone < 0.2)
      %text = "Establishing uplink with webcam network...";
   else if(%pctDone < 0.3)
      %text = "Downloading Credit-Card data...";
   else if(%pctDone < 0.4)
      %text = "Charging Vibrator energy cell...";
   else if(%pctDone < 0.5)
      %text = "Pingback satellite webcam uplink check...";
   else if(%pctDone < 0.6)
      %text = "Beginning primary oral system check...";
   else if(%pctDone < 0.7)
      %text = "Beginning secondary reproductive system check...";
   else if(%pctDone < 0.8)
      %text = "Downloading Pick up lines from tribal database...";
   else
      %text = "Starting Genital power-up sequence...";

   //Control::setText(ProgressText, "Warming up Whores's " @ %blockName @ " data...");
...
function dataFinished()
{
   ...
   Control::setValue(ProgressText, "<jc><f0>Warming up SEX oil...");
