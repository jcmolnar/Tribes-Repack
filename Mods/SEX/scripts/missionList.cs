...
// to work with random missions
// out of professional mod
// plasmatic
function MissionList::addMission(%mission)
{
   ...
   $Havoc::MType[$MLIST::EName[%ct]] = $MDESC::Type;
   //echo("$Havoc Mtype: " @ $MDESC::Type);
   //echo("Mission: " @ $MLIST::EName[%ct]);
   if($MDESC::Type != "Training")
      $MLIST::MissionList[0] = %ct @ " " @ $MLIST::MissionList[0];
   $MLIST::MissionList[%i] = %ct @ " " @ $MLIST::MissionList[%i];
   $TotalMissionList[%ct] = $MLIST::EName[%ct];
   $TotalMissions = %ct;
   return true;
}
