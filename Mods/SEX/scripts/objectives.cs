exec("game.cs");
exec(TriggerFunctions);	// redefining GroupTrigger:: functions cause people like to mess with em... -plasmatic	
...
{
	if(%player.outArea == 1 && !$outareadamage)
	{
		if(%count > 0)
		{
   ...
   else
   {
      ...
      if(%object.carryFlag == "")
      {
			...
			if(%playerClient.FlagCurse)
			 	schedule("PlayerFlagCurse("@%playerClient@");",2);
        		$flagAtHome[1] = false;
        		%this.atHome = false;
        		%this.carrier = %object;
        		%this.pickupSequence++;
