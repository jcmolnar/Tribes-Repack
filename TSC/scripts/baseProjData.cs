...
//--------------------------------------
GrenadeData NitrousBoost
{
   bulletShapeName    = "mortar.dts";
   explosionTag       = NitrousExp;
   collideWithOwner   = True;
   ownerGraceMS       = 250;
   collisionRadius    = 0.2;
   mass               = 20.0;
   ...
   totalTime          = 0.05;    // special meaning for grenades...
   liveTime           = 0.05;
   projSpecialTime    = 0.05;
...
function JammerBolt::onAcquire(%this, %player, %target)
{
	schedule("set_slow(" @ %target @ ");",0,%target);
	schedule("set_normal(" @ %target @ ");",5,%target);
	Bottomprint(Player::getClient(%target), "Your Afterburner has been jammed!");
}
