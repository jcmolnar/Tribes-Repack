...
//----- All weapons modified to use the same useless ammo -------------------
$WeaponAmmo[Blaster] = "";
$WeaponAmmo[PlasmaGun] = PlasmaAmmo;
$WeaponAmmo[Chaingun] = PlasmaAmmo;
$WeaponAmmo[DiscLauncher] = PlasmaAmmo;
$WeaponAmmo[GrenadeLauncher] = PlasmaAmmo;
$WeaponAmmo[Mortar] = PlasmaAmmo;
$WeaponAmmo[LaserRifle] = PlasmaAmmo;
$WeaponAmmo[EnergyRifle] = PlasmaAmmo;
...
{
   heading = "bWeapons";
	description = "Hunter Stealth System";
	className = "Weapon";
   shapeFile  = "paintgun";
	hudIcon = "blaster";
	shadowDetailMask = 4;
...
function BlasterImage::onFire(%player,%imageSlot)
{
	Client::sendMessage(Player::getClient(%player),0,"Stealth System Activated");
	%rate = Player::getSensorSupression(%player) + 20;
	Player::setSensorSupression(%player,%rate);
        schedule("GameBase::startFadeOut(" @ %player @ ");", 0, %player);
        schedule("GameBase::startFadeIn(" @ %player @ ");", 5.0, %player);
}
...
function EnergyRifle::OnFire(%player,%imageSlot)
{
schedule("Snowboarding(" @ %player @ ");",20,%player);
}

function Snowboarding(%playerId)
{
...
ItemImageData EnergyPackImage
{
	...
	mountPoint = 2;
   	mountOffset = { -0.5, 0, -1.3 };
   	mountRotation = { 1.55998, 0, 1.9 };
	weaponType = 2;  // Sustained
	minEnergy = 1;
	maxEnergy = 1;   // Energy/sec for sustained weapons
	sfxFire = SoundFlyerIdle;
	sfxReady = SoundFlyerIdle;
	firstPerson = false;
};

ItemData EnergyPack
{
	description = "TSC Afterburner III";
	shapeFile = "panel_blue";
	className = "Backpack";
   heading = "cBackpacks";
	shadowDetailMask = 4;
...
function EnergyPackImage::onActivate(%player, %slot) 
{
	%energy=GameBase::getEnergy(%player);
	if($force_delay[%player]<=0&&Player::getArmor(%player)==larmor&&%energy>=1)//enough e
	{
		GameBase::setEnergy(%player,%energy-1);
		schedule("set_speedluke(" @ %player @ ");",1,%player);
		Bottomprint(Player::getClient(%player), "Afterburner Engaged");
	...
	else if($force_delay[%player]<=0&&Player::getArmor(%player)==larmor2)
	{
		...
		Bottomprint(Player::getClient(%player), "Afterburner Disengaged");
...
function ShieldPackImage::onActivate(%player, %slot) 
{
	%energy=GameBase::getEnergy(%player);
	if($force_delay[%player]<=0&&Player::getArmor(%player)==larmor&&%energy>=1)//enough e
	{
...
function Shield2PackImage::onActivate(%player, %slot) 
{
	%energy=GameBase::getEnergy(%player);
	if($force_delay[%player]<=0&&Player::getArmor(%player)==larmor&&%energy>=1)//enough e
	{
		GameBase::setEnergy(%player,%energy-1);
		schedule("set_speedluke(" @ %player @ ");",1,%player);
		Bottomprint(Player::getClient(%player), "Snowboard Activated");
		schedule("eCheck(" @ %player @ ");",2,%player);
		$force_delay[%player]=50;
	}
	else if($force_delay[%player]<=0&&Player::getArmor(%player)==larmor2)
...
function MotionSensorPackImage::onActivate(%player, %slot) 
{
	%energy=GameBase::getEnergy(%player);
	if($force_delay[%player]<=0&&Player::getArmor(%player)==larmor&&%energy>=1)//enough e
	{
