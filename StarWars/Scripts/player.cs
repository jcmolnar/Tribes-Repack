...
function Player::onKilled(%this)
{
	...
	for (%i = 0; %i < 8; %i = %i + 1) {
		%type = Player::getMountedItem(%this,%i);
		if (%type != -1) {
			if (%i != $WeaponSlot || !Player::isTriggered(%this,%i) || getRandom() > "1.0") 
...
function Player::onDamage(%this,%type,%value,%pos,%vec,%mom,%vertPos,%quadrant,%object)
{
	...
	// Lightsaber deflecting/blocking code
	%mountGun = Player::getMountedItem(%this, $WeaponSlot);
	if(%mountGun == BSaber || %mountGun == RSaber || %mountGun == MSaber || %mountGun == GSaber) {

//		if(%quadrant != "back_left" && %quadrant != "back_right") 
// Test Player quadrant code
		if(%quadrant == "middle_front" || %quadrant == "middle_middle" || %quadrant == "front_left" || %quadrant == "front_right") 

			{
			if(GameBase::getEnergy(%this) > 20) {
				if(%type == $LaserDamageType) { // Deflect blaster shot
					Client::sendMessage(%shooterClient, 1, Client::getName(%damagedClient) @ " has deflected your bolt!");
					Projectile::spawnProjectile("TrooperBlast", GameBase::getMuzzleTransform(%this), %this, Item::getVelocity(%this));
					
					%redhit = GameBase::getEnergy(%this);
					%redhit2 = (%redhit - 15);
					GameBase::setEnergy(%redhit2);
					
					%value = 0.05;
				}
				if(%type == $PlasmaDamageType) { // Deflect blaster shot
					Client::sendMessage(%shooterClient, 1, Client::getName(%damagedClient) @ " has deflected your bolt!");
					Projectile::spawnProjectile("NabooBlast", GameBase::getMuzzleTransform(%this), %this, Item::getVelocity(%this));
									
					%greenhit = GameBase::getEnergy(%this);
					%greenhit2 = (%greenhit - 15);
					GameBase::setEnergy(%greenhit2);
					
					%value = 0.05;
				}

				if(%type == $MSaberDamageType || %type == $BSaberDamageType || %type == $GSaberDamageType || %type == $RSaberDamageType) 
				{  
					Client::sendMessage(%shooterClient, 1, Client::getName(%damagedClient) @ " blocked your thrust!");
					%sabhit = GameBase::getEnergy(%this);
					%sabhit2 = (%sabhit - 10);
					GameBase::setEnergy(%sabhit2);
					%value = 0.05;
				}
			}	
		}
		...
		if($teamplay && %damagedClient != %shooterClient && Client::getTeam(%damagedClient) == Client::getTeam(%shooterClient) ) {
			if (%shooterClient != -1) {
			   ...
			   if ((%curTime - %this.DamageTime > 3.5 || %this.LastHarm != %shooterClient) && %damagedClient != %shooterClient && $Server::TeamDamageScale > 0) {
					if ((%type != $MineDamageType) && (%type != $ElectricityDamageType)) 
					{
