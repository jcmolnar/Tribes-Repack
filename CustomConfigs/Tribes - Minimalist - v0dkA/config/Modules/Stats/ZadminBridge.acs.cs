...
function remoteonNadeJump( %sv, %cl, %speed ) {
	
	Event::Trigger( eventOnNadeJump, %cl, %speed );

}

function remoteMidAirDisc( %sv, %shooter, %victim, %time ) {
    
    $zadmin::PlayerMATime[ %victim ] = getSimTime();
	Event::Trigger( eventMidAirDisc, %shooter, %victim );
}

function remoteMidAirNade( %sv, %shooter, %victim ) {
    
    $zadmin::PlayerMANadeTime[ %victim ] = getSimTime();
	Event::Trigger( eventMidAirNade, %shooter );
}

function remoteFlagInterception( %sv, %teamid, %cl ) {
	
	Event::Trigger( eventFlagInterception, %teamid, %cl );

}

function remoteFlagCatch( %sv, %teamid, %cl ) {
	
	Event::Trigger( eventFlagCatch, %teamid, %cl );

}

function remoteBodyBlock( %sv, %shooter, %victim ) {
	
	Event::Trigger( eventBodyBlock, %shooter, %victim );

}

function remoteDamageDealt( %sv, %shooter, %victim, %damage ) {
	
	Event::Trigger( eventDamageDealt, %shooter, %victim, %damage );

}

function remoteTeamDamageDealt( %sv, %shooter, %victim, %damage ) {
	
	Event::Trigger( eventTeamDamageDealt, %shooter, %victim, %damage );

}

function remoteHitCG( %sv, %cl ) {
	Event::Trigger( eventHitCG, %cl );
}

function remoteCountdownStarted( %sv, %time ) {
    Event::Trigger( eventCountdownStarted, %time );
}

function remoteMissionComplete( %sv ) {
    Event::Trigger( eventMissionComplete );
}

function remoteFlagClutchReturn( %sv, %cl, %flagTeam ) {
	Event::Trigger( eventFlagClutchReturn, %cl, %flagTeam );
}

function remoteFlagEgrab( %sv, %cl, %flagTeam ) {
	Event::Trigger( eventFlagEGrab, %cl, %flagTeam );
}

function remotePlayerClunk( %sv, %cl ) {
	Event::Trigger( eventPlayerClunk, %cl );
}
