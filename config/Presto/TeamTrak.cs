...
//
//
//	Edited for MT Parser Compliency October 2000 by Wizard_TPG
//
...
function Team::Reset() {
	...
	$TeamData::TeamCalcTeamName = true;
...
{
	if($TeamData::TeamNum[%teamNum, name] != %name)
		$TeamData::Reinit[%teamNum] = true;

	if($TeamData::TeamNum[%teamNum, name] != "")
		if($TeamData::Reinit[%teamNum])
			return;
	...
	for(%i=0;%i<2;%i++)
	{
		if($TeamData::TeamNum[%i, name] == "")
			%counter++;
	}
	if(%counter > 0)
		$TeamData::TeamCalcTeamName = true;
	else
		$TeamData::TeamCalcTeamName = false;
...
function Team::ParseFlagMessage(%action)
{
	...
	if (!$TeamData::TeamCalcTeamName)
		return;
	Team::SetName(%teamFlag, Match::Result(%m, "t"));
	return;
	}

function TeamTrak::MatchStart(%client, %msg)
{
	if (%client != 0)
		return;
...
function TeamTrak::Dropped(%client,%msg)
{
	if (%client != 0)
		return;
...
function TeamTrak::FlagEventTaken (%client, %msg)
{
	if (%client != 0)
		return;
//	%msg=string::getSubStr (%msg, string::len(%msg)-11);
	if (Match::ParamString(%msg, "%p took the %t flag! "))  //I have no idea why, but this one requires the wav bit as well.
		{
...
function TeamTrak::FlagEventReturned (%client, %msg)
{
	if (%client != 0)
		return;
//	%msg=string::getSubStr (%msg, string::len(%msg)-16);
...
function TeamTrak::FlagEventCaptured (%client, %msg)
{
	if (%client != 0)
		return;
//	%msg=string::getSubStr (%msg, string::len(%msg)-17);
...
function TeamTrak::FlagEventDropped (%client, %msg)
{
	if (%client != 0)
		return;
...
function TeamTrak::GetInitialTeam(%team, %name)
{
	%teamNum = %team - 1;
	Team::SetName(%teamNum, %name);
	return;
}

function remotebwadmin::setObjList(%client, %i, %num, %objName, %type, %status)
{
	//For others to attach to
	Event::Trigger(eventBwadminSetObjList, %client, %i, %num, %objName, %type, %status);

	if(%type == "flag")
	{
		//Update TeamName Info
		%teamname = "";
		for(%count=0;%count<String::getWordCount(%objName)-1;%count++)
		{
			if(%i==0)
				%teamname = getWord(%objName,%count);
			else
				%teamname = %teamname @ " " @ getWord(%objName,%count);
		}
		$TeamData::Team[%teamname] = %i;
		%teamNum = Team::GetByName(%teamname);
		$TeamData::Reinit[%teamNum] = true;

		//Update objectives info on connection mid-game
		if(%status == "home")
			Team::MoveFlag(%teamNum, $Trak::locationHome);
		else if(%status == "dropped")
			Team::MoveFlag(%teamNum, $Trak::locationField);
		else
			Team::MoveFlag(%teamNum, %status);
	}
}


msg::onMatch ("Match starts in * seconds.", "TeamTrak::MatchStart(%client,%msg);");
msg::onMatch ("Match started.", "TeamTrak::MatchStart(%client,%msg);");
msg::onMatch ("dropped.", "TeamTrak::Dropped(%client,%msg);");
msg::onMatch ("took the", "TeamTrak::FlagEventTaken(%client,%msg);");            //"took the * flag! "
msg::onMatch ("returned the * flag!", "TeamTrak::FlagEventReturned(%client,%msg);");
...
Event::Attach(eventClientTeamAdd, TeamTrak::GetInitialTeam);
