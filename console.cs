...
for(%i = 1; $cargv[%i] != ""; %i++)
{
	...
	else if($cargv[%i] == "-dedicated")
	{
		...
		if(%mission != "")
		{
			%firstChar = string::getsubstr(%mission,0,1);
			//if(%mission != "-mod" && %mission != "+exec" && %mission != "+password" && %mission != "-host" && %mission != "+maxplayers" && %mission != "+record"){
			if(string::compare(%firstChar,"-") != 0 && string::compare(%firstChar,"+") != 0) {
				$HostMission = %mission;
				%i++;
			}
		}
...
if($repackver == "")
   ...
   else
   {
	...
	newObject("hosedtexsgsg", SimVolume, "prefabs\\hosed3.vol");
	newObject("othermoofdgd", SimVolume, "prefabs\\blacksmith.vol");
}
