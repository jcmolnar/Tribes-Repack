...
function createAI(%aiName, %markerGroup, %name)
{
	...
	{
	      	decho("Failure spawning bot:");
		decho("%aiName: " @ %aiName);
		decho("%armor: " @ %armor);
		decho("%guardtype: " @ %guardtype);
		decho("%spawnPos: " @ %spawnPos);
		decho("%spawnRot: " @ %spawnRot);
		decho("%name: " @ %name);
		return -1;
      }
}
...
function AI::Periodic(%aiName)
{
	...
	//=================================================================
	//0 thru 4 = animation 10, 5 thru 10 = animation 11, else do nothing
	//=================================================================
...
function SpawnAI(%newName, %displayName, %aiSpawnPos, %commandIssuer, %loadout)
{
	...
	if(%retval != -1)
	{
		...
		if(GetWord(%commandIssuer, 0) == "TempSpawn")
		{
			...
			if(String::ICompare(%team, "default") == 0)
				%team = $TeamForRace[$BotInfo[%newName, RACE]];
			GameBase::setTeam(%aiId, %team);
...
function HardcodeAIskills(%aiId)
{
	...
	%lvl = fetchData(%aiId, "LVL")-1;

	%ns = getNumSkills();
	%a = $autoStartupSP + round($initSPcredits / %ns) + round((%lvl * $SPgainedPerLevel) / %ns);
	for(%i = 1; %i <= %ns; %i++)
		AddSkillPoint(%aiId, %i, %a);

	//==== HARDCODED SKILLS TO ENSURE CHALLENGING BOTS ============
	$PlayerSkill[%aiId, $SkillMelee] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillLightsabers] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillBashing] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillDodging] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillArchery] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillOffensiveCasting] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillDefensiveCasting] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillNeutralCasting] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillEnergy] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillCharisma] = $SkillCap;
	$PlayerSkill[%aiId, $SkillWeightCapacity] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);
	$PlayerSkill[%aiId, $SkillEndurance] = ( (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel) ) / 2;
	$PlayerSkill[%aiId, $SkillSenseHeading] = (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel);

	%a = (  (getRandom() * $SkillRangePerLevel) + (%lvl * $SkillRangePerLevel)  ) / 2;
...
function InitTownBots()
{
	...
	if(%group != -1)
	{
		...
		for(%i = 0; %i <= %cnt - 1; %i++)
		{
			...
			%townbot = newObject("", "StaticShape", $BotInfo[%name, RACE] @ "TownBot", false);
...
function RotateTownBotOldAndSilly(%id, %rot)
{
