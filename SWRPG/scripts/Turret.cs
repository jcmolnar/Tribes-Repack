...
{
	if($owner[%this] == "")	//no owner, so it is a regular turret
	{
		%retval = "True";
	}
	else
	{
		//there is an owner, so don't let turret hit him or his buddies
		%name = Client::getName(%target);
		
		if($owner[%this] == %name)
			%retval = "False";
            else if(IsInCommaList($grouplist[$owner[%this]], %name))
			%retval = "False";
		else
			%retval = "True";
	}
...
//	%retval = "False";
//
//	for(%i=1; %i <= $turretNum; %i++)
//	{
//		%dist = Vector::getDistance($turretPos[%i], %clpos);
//		%turret = $turretIndex[%i];
//		if(%dist <= 30)
//		{
//
//			//turret is within range, so consider firing
//			if($owner[%turret] == "")	//no owner, so it is a regular turret
//			{
//				%retval = "True";
//			}
//			else
...
//				%name = Client::getName(%damagedClient);
//		
//				if($owner[%turret] == %name)
//					%retval = "False";
//		            else if(IsInCommaList($grouplist[$owner[%turret]], %name))
//					%retval = "False";
//				else
//					%retval = "True";
//			}
